...
def get_gspread_client():
    # Retrieve the secret from Colab userdata
    vault_json_string = userdata.get('GCP_SERVICE_ACCOUNT_JSON')
    if not vault_json_string:
        raise ValueError("Missing GCP_SERVICE_ACCOUNT_JSON in Colab secrets.")
...
def run_training_pipeline():
    ...
    output_dir = os.path.join("prediction service", "artifacts")
    ...
    model.fit(X_train.values, y_train.values) # Using .values for clean evaluation array conversions

    # Convert Scikit-Learn Model to ONNX Form
    print("🔄 Converting model matrix architecture to ONNX format...")
    # Our model accepts 4 float features: ['Cheque Amount', 'month_encoded', 'days_in_limbo', 'agent_risk_score']
